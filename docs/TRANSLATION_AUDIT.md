...
- Structural `enUS`/`deDE` tip mismatches: **20**
...
- `20` suspicious structural mismatch(es) were found while comparing explicit `deDE` overrides against `enUS`.

Details:
- `tipsMap_deDE[17536]` tip 1 does not match `enUS` metadata (`tip_id`, `type`, or `weight`).
