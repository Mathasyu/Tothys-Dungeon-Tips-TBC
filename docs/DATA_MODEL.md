...
- `tipsMap_deDE` is now a fully materialized explicit table in the file
...
- a normalized future model is now generated by script into `docs/NORMALIZED_CONTENT_MODEL.json`
  - this is a migration target and review artifact
  - runtime still uses the current shipped Lua tables for now
...
The long-term target is a single generic model instead of parallel locale tables such as
`tipsMap_enUS` and `tipsMap_deDE`.

Target principles:

- no locale suffixes in top-level variable names
- one logical tip object per tip
- localized fields live inside the object itself
- instance keys are stable identifiers
- local tip ids are only unique within their NPC scope
- legacy full tip ids are preserved during migration for reversibility
- new expansions, instances, NPCs, and locales should be additive only
...
addon.db = {
  ...
  instances = {
    hellfire_ramparts = {
      ...
      npcIDs = { 17306, 17308, 17536, 17537 },
    },
  },
  ...
  instanceTips = {
    hellfire_ramparts = {
      {
        id = "01",
        legacy_id = "ramparts_instance_01",
        type = "Important",
        weight = 20,
        text = {
          enUS = "...",
...
- the unique path becomes `instanceKey -> npcID -> localTipID`
- `legacy_id` is kept until runtime, export, and import are fully migrated
- adding a new locale later should only mean adding `text.frFR`, `name.esES`, and so on
- adding a new NPC should not require adding multiple parallel locale tables
- this structure is better suited for later in-game authoring, import/export, and expansion-aware variants

Current migration tooling:

- `scripts/export_normalized_model.py`
- `docs/NORMALIZED_CONTENT_MODEL.json`

Current migration status:

- the current shipped DB can now be exported into the normalized target model by script
- runtime has not yet been switched over to read from that normalized model
